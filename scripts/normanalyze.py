...
normdata = {}
for field in ["branch","stmt","fail"]:
    if (not anyFailed) and (field == "fail"):
        continue
    maxd = 0
    for cat in ["random","LOC","swarm","LOC+swarm","LOC+GA","GA"]:
        if cat not in normdata:
            normdata[cat] = {}
        if max(data[cat][field]) > maxd:
            maxd = (max(data[cat][field])) * 1.0
    print "MAX",field,maxd
    for cat in ["random","LOC","swarm","LOC+swarm","LOC+GA","GA"]:
        normdata[cat][field] = map(lambda x: x / maxd, data[cat][field])

print normdata

for cat in normdata:
    with open("norm."+cat+".alldata",'a') as file:
        branches = map(str,normdata[cat]["branch"])
        stmts = map(str,normdata[cat]["stmt"])
        if anyFailed:
            failures = map(str,normdata[cat]["fail"])
        else:
            failures = ["NA"]*len(branches)
        for i in xrange(len(branches)):
            file.write(branches[i])
            file.write(" ")
            file.write(stmts[i])
            file.write(" ")
            file.write(failures[i])
            file.write("\n")
...
bb = plt.boxplot([data["random"]["stmt"],data["LOC"]["stmt"],data["swarm"]["stmt"],data["LOC+swarm"]["stmt"],data["GA"]["stmt"],data["LOC+GA"]["stmt"]],manage_xticks=False)
...
if anyFailed:
    ...
    bb = plt.boxplot([data["random"]["fail"],data["LOC"]["fail"],data["swarm"]["fail"],data["LOC+swarm"]["fail"],data["GA"]["fail"],data["LOC+GA"]["fail"]],manage_xticks=False)
